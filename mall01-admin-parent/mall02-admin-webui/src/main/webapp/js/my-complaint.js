...
function getPageInfoRemote() {
    ...
    var ajaxResult = $.ajax({
        url: "get/complaint/page/info.json",
        type: "post",
        data: {
...
function fillTableBody(pageInfo) {
    ...
    var complaintPageBody = $("#complaintPageBody");
    // 清除tbody原来的内容
    complaintPageBody.empty();
    //这里清空是为了当查询不到结果时不显示底部分页导航条
    $("#Pagination").empty();
    // 判断pageInfo是否有效
    if (pageInfo == null || pageInfo.list == null || pageInfo.list.length === 0) {
        complaintPageBody.append("<tr><td colspan='8' align='center'>抱歉！没有查询到您搜索的数据</td></tr>");
        return;
    }
    // 使用pageInfo的list属性填充tBody
    for (let i = 0; i < pageInfo.list.length; i++) {

        let complaint = pageInfo.list[i];
        let id = complaint.id;
        let userId = complaint.userId;
        let productId = complaint.productId
        let content = complaint.content;
        let productName = complaint.productName;
        let status = complaint.status;


        let numberTd = "<td>" + (i + 1) + "</td>";
        let idTd ="<td>" + id + "</td>";
        let userIdTd ="<td>" + userId + "</td>";
        let productIdTd = "<td>" + productId + "</td>";
        let contentTd = "<td>" + content + "</td>";
        let productNameTd = "<td>" + productName + "</td>";
        let statusTd = "<td>" + status + "</td>";


        var checkBtn = "<button id='" + id + "' type=\"button\" class=\"btn btn-success btn-xs changeStatus\"><i class=\" glyphicon glyphicon-check\"></i></button>";
        ...
        var buttonTd = "<td>"+checkBtn+ "</td>"
        var tr = "<tr>"+numberTd +idTd + userIdTd + productIdTd+ productNameTd+contentTd+statusTd+buttonTd+ "</tr>";

        complaintPageBody.append(tr);
    }
    // 生成分页导航条
